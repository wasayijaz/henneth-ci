...
def _analogue_outcomes(event, all_events, sectors, histories):
    target_day = parse_date(event.get("effective_date")); etype = event.get("event_type"); sym = event.get("symbol")
    ...
    for candidate in all_events:
        ...
        cday = parse_date(candidate.get("effective_date"))
...
def build():
    ...
    studies = {}
    for event in events:
        sym = event.get("symbol"); rows = histories.get(sym) or []; day = parse_date(event.get("effective_date")); baseline = last_before(rows, day) if day else None; last_day = _last_day(rows)
