...
| Financial engine assumptions | `scripts/build_financial_engine_assumptions.py` -> `state/company_intel/financial_engine_assumptions.json`; `scripts/import_owner_financial_assumptions.py`; manual `scripts/approve_owner_financial_assumptions.py` | deterministic source-labelled market operands (`current_price`, `shares_out`) plus explicitly non-forecast historical reference cases for revenue growth and net margin from three qualified annual facts; owner-approved forward/valuation records remain separate and required by formal engines, with private Supabase drafts approved only by an explicit server-side append-copy handoff |
