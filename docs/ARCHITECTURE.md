...
| Financial engine assumptions | `scripts/build_financial_engine_assumptions.py` -> `state/company_intel/financial_engine_assumptions.json` | deterministic source-labelled market operands (`current_price`, `shares_out`) plus explicitly non-forecast historical reference cases for revenue growth and net margin from three qualified annual facts; owner-approved forward/valuation records remain separate and required by formal engines |
