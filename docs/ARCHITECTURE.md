...
| CI release integrity | `scripts/build_ci_artifact_integrity.py`, `scripts/check_ci_artifact_integrity.py`, `.github/workflows/ci-production-release.yml`, `scripts/check_ci_vercel_deployment_commit.py` | seals the generated CI artifacts to one source commit/cutoff and permits production only through a protected, tested-preview promotion whose live Vercel metadata resolves to that exact commit and deployment id; external Vercel/GitHub configuration remains a required owner-controlled boundary |
