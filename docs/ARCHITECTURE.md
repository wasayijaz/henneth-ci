...
| EOD refresh scope (which symbols get repriced this run) | `scripts/fetch_history.py` `_pick()` + `main()` — a FULL SWEEP: every symbol that has a series is repriced EVERY run, fetched concurrently (`WORKERS`, `ThreadPoolExecutor`), so one honoured cron tick keeps the whole universe ≤1 day old. `DEADLINE_S` guards the wall clock; a SEPARATE `NEW_PROBE_PER_RUN` round-robin onboards symbols with no series yet. Deliberately NOT a per-run slice — the cron load-sheds and a slice never finishes a lap (see GOTCHAS "The cron is a wish") | every price consumer; reported in `state/history_meta.json` |
| Refresh ordering clock (only matters if `DEADLINE_S` cuts a run short) | `state/history_meta.json` `last_attempt` — a persisted symbol→timestamp map, stamped once a fetch completes so the stalest lead the next run and a cut-short run skips only the freshest. **Never** a filesystem mtime: `actions/checkout` rewrites every file alphabetically on each cloud run | `fetch_history.py` `_pick()`; audited by `preflight.py`'s stuck-rotation + `skipped_deadline` WARNs |
...
- The homepage (`site/src/pages/index.astro`) uses the approved Concept D visual language as native Astro markup. The hero stays in the page; `site/src/components/home/ConceptDPostHero.astro` owns sixteen post-hero chapters in source, visual and reading order: editorial statement, reader starting points, production line, pricing, research stories, edition surfaces, Desk Room, autonomous cycle, illustrative case, illustrative scorecard, research principles (including the removed share-count example), tip-versus-research comparison, reader testimonials, FAQ, daily-read account CTA and final CTA. Styles and behavior remain isolated in `site/src/styles/home-concept-d-posthero.css` and `site/src/scripts/home-concept-d-posthero.ts`. The daily-read CTA uses the configured `appUrl`; the homepage does not implement a second signup flow. Ticker, case and scorecard mockups remain explicitly illustrative and are never presented as current research. The page keeps `Base.astro` as its shell, reads only public marketing extracts and does not embed the standalone bundle or add a second router.
- Concept D motion is split at the same seam. The hero video ping-pongs between `site/public/concept-d/hero-loop.mp4` and `hero-loop-reverse.webm`, applies the scroll-driven blur and veil, rotates the featured desk argument and runs the sector marquee. The muted, `playsinline` video is available at desktop, tablet and mobile widths, while the foreground remains usable if media or autoplay is unavailable. Post-hero motion includes scroll reveals, the production line, filter states, investor-path progression, Desk Room scenarios, two low-power WebGL fields, the illustrative case sequence, principle verification, share-count deletion and comparison/FAQ states. Reduced-motion handling pauses the hero video, skips its scroll loop, settles post-hero animated state and renders one static WebGL frame. Compact layouts turn long card and stage collections into horizontal snap rails.
- `site/src/components/Header.astro` is the only marketing-site navigation owner. `Base.astro` mounts the shared glass presentation on every route, including the homepage, so navigation spacing and scroll behavior stay homogeneous. On desktop/tablet one fixed glass pill appears after 320 px of scroll. At 760 px and below the same component becomes a fixed glass mobile bar with a left-edge drawer over a full-viewport dimmed/blurred scrim. The drawer preserves the shared focus trap, Escape/scrim/link close, focus restoration and HTML/body scroll lock; its viewport overlay is a sibling of the filtered header so browser containing-block behavior cannot constrain it to the bar. Header, page wraps, homepage sheet and footer all share the 1180 px `--maxw` canvas, with 24 px desktop/tablet and 18 px mobile gutters below the homepage hero.
