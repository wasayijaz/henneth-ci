...
- The homepage (`site/src/pages/index.astro`) uses the Concept D visual language as native Astro markup. The approved hero stays in the page; the complete post-hero sequence is owned by `site/src/components/home/ConceptDPostHero.astro`, with its styles and behavior isolated in `site/src/styles/home-concept-d-posthero.css` and `site/src/scripts/home-concept-d-posthero.ts`. The sequence contains fifteen chapters: pricing, production pipeline, filterable surfaces, editorial statement, investor paths, surface tour, Desk Room, immersive cycle, published/resolved/archive case, public scorecard, principles, share-count deletion, tip-versus-research comparison, FAQ, signup preview and final CTA. It keeps `Base.astro` as the shell and reads only public extracts; it does not embed the 8.9 MB standalone bundle or add a second router.
- Concept D motion is split at the same seam. The hero video ping-pongs between `site/public/concept-d/hero-loop.mp4` and `hero-loop-reverse.webm`, applies the reference scroll-driven blur/veil values, rotates the featured desk argument and runs the sector marquee. Post-hero motion includes scroll reveals, the animated production line, card/filter states, scroll-reactive statement, investor-path progression, autonomous Desk Room scenarios, two low-power WebGL fields, the case/archive sequence, principle verification, share-count deletion and comparison/FAQ states. Reveal, rule and share observers are one-shot; Desk Room and case loops start on first visibility and run until page teardown; WebGL layers are visibility-gated with a 120 px root margin and a DPR cap of 1.5. `prefers-reduced-motion` settles the content, skips loops and scroll effects, and renders one static WebGL frame. The mobile breakpoint intentionally removes the hero video layer (matching the supplied Mobile v2 reference) while retaining the post-hero interactions and horizontal surface tours. The standalone artifact is never decoded at runtime.
- `site/src/components/Header.astro` is the only marketing-site navigation owner. `Base.astro` mounts its default glass presentation on every route, including the homepage, so navigation spacing and scroll behavior stay homogeneous; `Base.astro` still exposes `navMode="immersive"` for an explicitly immersive page. On desktop/tablet one fixed glass pill appears after 320 px of scroll. At 760 px and below the same component becomes a fixed glass mobile bar with a left-edge drawer over a full-viewport dimmed/blurred scrim. The drawer preserves the shared focus trap, Escape/scrim/link close, focus restoration and HTML/body scroll lock; its viewport overlay is a sibling of the filtered header so browser containing-block behavior cannot constrain it to the bar. Header, page wraps, homepage sheet and footer all share the 1180 px `--maxw` canvas, with 24 px desktop/tablet and 18 px mobile gutters below the homepage hero.
